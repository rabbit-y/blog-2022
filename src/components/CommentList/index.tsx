import { Row, Col, Pagination, Avatar } from 'antd';
    ...
    <div className="h-comment-list-box">
      ...
      <div className="h-comment-list" id="goCommentList">
        {list?.map((item, index) => {
          ...
          const qqAvatar =
            qq.length > 1 ? `https://q1.qlogo.cn/g?b=qq&nk=${qq[0]}&s=100` : item.avatar ? item.avatar : '';
          return (
            <Row wrap={false} gutter={[20, 20]} key={index} className="h-comment-item">
              <Col flex="none">
                <Avatar className="h-comment-avatar" src={qqAvatar}>
                  {item.nickName}
                </Avatar>
              </Col>
              <Col flex="auto">
                      ...
                      {item.children.map((items, indexs) => {
                        ...
                        return (
                          <Row wrap={false} gutter={[20, 20]} key={indexs} className="h-comment-item">
                            <Col flex="none">
                              <Avatar className="h-comment-avatar" src={myAvatar} />
                            </Col>
                            <Col flex="auto">
                              <div className="h-comment-reply">
                                <div className="h-comment-reply-top">
